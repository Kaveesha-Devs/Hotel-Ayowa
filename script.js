...
// Book now button scroll to contact or open booking widget
const bookBtns = document.querySelectorAll('.book-btn, .cta-btn');

bookBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        const bookingWidget = document.querySelector('.booking-widget');
        if (bookingWidget) {
            // Scroll to show booking widget
            window.scrollTo({
                top: 0,
                behavior: 'smooth'
            });
            // Highlight the booking widget
            bookingWidget.style.animation = 'pulse 1s ease';
            setTimeout(() => {
...
// Handle booking form submission - Open Modal
if (bookingSubmitBtn) {
    bookingSubmitBtn.addEventListener('click', function() {
        const checkIn = document.getElementById('checkIn').value;
        const checkOut = document.getElementById('checkOut').value;
        const guests = document.querySelector('.booking-input[name="guests"]') ||
                      document.querySelectorAll('.booking-input')[2];
        const roomType = document.querySelector('.booking-input[name="roomType"]') ||
                        document.querySelectorAll('.booking-input')[3];

        const guestsValue = guests.value || guests.options[guests.selectedIndex].text;
        const roomTypeValue = roomType.value || roomType.options[roomType.selectedIndex].text;
        ...
        // Store booking data
        bookingData = {
            checkIn: checkInDate.toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }),
            checkOut: checkOutDate.toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }),
            checkInRaw: checkIn,
            checkOutRaw: checkOut,
            nights: nights,
            guests: guestsValue,
            roomType: roomTypeValue,
            pricePerNight: roomPrices[roomTypeValue] || 250,
            totalPrice: (roomPrices[roomTypeValue] || 250) * nights
        };
...
if (confirmBtn && fullBookingForm) {
    confirmBtn.addEventListener('click', function(e) {
        ...
        updateConfirmationModal(completeBooking);
...
function updateConfirmationModal(booking) {
    // Update booking reference
    const refElement = document.querySelector('.ref-number');
    if (refElement) {
        refElement.textContent = booking.bookingRef;
    }
...
// Close confirmation and reload page
const closeConfirmationBtn = document.querySelector('.confirmation-modal .modal-close');
